...
**2.** Confirm file integrity by checking the md5sum.
...
- You can explain in one sentence why `wc -l` is worth running before `cat`
...
**4.** How would you write the output of one of these commands to a new file instead of just printing it to the screen? Try it.

**You are done when:**
- You can preview the start and end of `mendel.txt` with a custom number of lines
- You have redirected the output of `head` or `tail` into a new file and confirmed it with `cat`

### Exercise 4: Reading Large Files

**Goal:** Inspect the contents of a large file using a reader or editor.
...
* Exit with `q`
* Perform forward searches (`/`) and reverse searches (`?`) - `n` jumps to next, `N` to previous hit
* `g` jumps to the start of the document, `G` to the end
...
### Exercise 5: Searching with grep
...
**3.** Prepare a file named `mendel_generation.txt` that contains the first 5 lines of `mendel.txt`, followed by all lines containing "Generation". 
...
**5**. Use a clear filename. For Bash scripts, the `.sh` extension is commonly used:
```
my_script.sh
```

The extension is not strictly required, but it makes the purpose of the file easier to recognise.
...
This gives the file permission to be run as a program. After that, you can execute it directly:
```
./my_script.sh
```
...
**8.** Create a script named `hardcoded.sh` that repeats the command pair above for each of these words: `Hybriden`, `Generation`, `Pflanze`, `Samen`, `Merkmal`. For each word, the script should create a *separate* output file containing the first 5 lines of `mendel.txt` followed by all lines containing that word. (Code is "hardcoded" when values are fixed directly in the script, so changing them means editing the script itself.)
...
**1.** Create a file called `variables.sh`.
...
head -n 5 mendel.txt > mendel_${grep_word}.txt
grep "${grep_word}" mendel.txt >> mendel_${grep_word}.txt
```
`$1` refers to the first argument passed to the script when you run it.  
You can also write scripts that accept multiple arguments, those would be accessed with `$2`, `$3`, and so on, in the code.

**3.** Make it executable and run it with an argument:
```
chmod +x variables.sh
bash variables.sh Generation
```
`$1` is replaced by `Generation`, so the script searches for "Generation" and writes the result to `mendel_Generation.txt`.

**4.** Run it again with a different word, use the up arrow on the command line to recover the last executed command.

**You are done when:**
- `variables.sh` runs successfully with at least two different words as arguments
- Each run produces the expected output file without you editing the script
- You can explain what `$1` refers to

**Extension - if you finish early:**
- What other elements in the script could be replaced by a variable? 
- Try running `variables.sh` with no argument at all. What happens to `${grep_word}`?

---

### Exercise 8: For Loops

**Goal:** Use a `for` loop to repeat a command automatically for a list of values, instead of running the script by hand each time.

**1.** A `for` loop repeats the same command block for several values in turn. Look at this example:
```
#!/bin/bash

for grep_word in "Hybriden" "Generation" "Pflanze" "Samen" "Merkmal"
do
...
```
The variable `grep_word` takes one value at a time - first `Hybriden`, then `Generation`, and so on - and the loop stops after the last item.
...
**Extension - if you finish early:**
- Add an `echo "All searches complete."` line after the `done` keyword. When does it print, relative to the loop?
...
**1.** If you update your code and expect the outputs not to be affected, you can run `diff file1.txt file2.txt` to identify any differences between 2 files.
...
**3.** Now run `diff` on 2 files where you expect differences. For example the `grep` output for "Hybride" vs "Hybriden". How does the output look?
